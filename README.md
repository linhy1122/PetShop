...
- [🏗 项目架构](#-项目架构)
- [🛠 技术栈](#-技术栈)
  - [后端 (`pet-shop-server`)](#后端-pet-shop-server)
  - [Web 前端 (`pet-shop-web`)](#web-前端-pet-shop-web)
  - [小程序 (`pet-shop-mobile`)](#小程序-pet-shop-mobile)
- [📁 项目结构](#-项目结构)
- [🧩 功能矩阵](#-功能矩阵)
  - [🏪 商品系统](#-商品系统)
  - [🛒 交易系统](#-交易系统)
  ...
  - [🔧 管理后台 (Web)](#-管理后台-web)
- [💡 核心亮点](#-核心亮点)
  - [1. 协同过滤个性化推荐](#1-协同过滤个性化推荐)
  - [2. AI 智能客服（Prompt-based RAG）](#2-ai-智能客服prompt-based-rag)
  - [3. 微信小程序跨端兼容](#3-微信小程序跨端兼容)
  - [4. 会员等级与动态定价](#4-会员等级与动态定价)
  - [5. 推荐算法优化](#5-推荐算法优化)
  - [6. 管理后台数据统计](#6-管理后台数据统计)
  - [7. 完整订单生命周期](#7-完整订单生命周期)
- [🚀 快速启动](#-快速启动)
  - [环境要求](#环境要求)
  - [第一步：数据库初始化](#第一步数据库初始化)
  - [第二步：配置并启动后端](#第二步配置并启动后端)
  - [第三步：启动 Web 前端](#第三步启动-web-前端)
  - [第四步：启动小程序](#第四步启动小程序)
- [📡 API 文档](#-api-文档)
  - [接口一览](#接口一览)
  - [统一响应格式](#统一响应格式)
...
| 库存预警 | ✅ | ✅ | 库存不足时显示警告 |
| 下单结算 | ✅ | ✅ | 选择地址 + 备注 + 提交订单 |
| 会员动态折扣 | ✅ | ✅ | 下单自动按会员等级计算折扣价（95折/9折/85折） |
| 订单折扣明细 | ✅ | ✅ | 订单详情展示原价、折扣金额和实付对比 |
...
| 密码修改 | ✅ | ✅ | 原密码验证 |
| 收货地址 | ✅ | ✅ | 增删改查 + 默认地址 |
| 会员等级展示 | ✅ | ✅ | 个人中心显示等级标签（银=银色/金=亮金/钻石=宇宙紫光效） |
| 会员自动升级 | ✅ | ✅ | 累计消费达 ¥1000/5000/20000 自动晋升 |
...
| 视频管理 | 视频上传、编辑、删除 |
| 数据统计 | 营收统计（今日GMV/实收/本月退款）、7日趋势折线图、订单状态饼图 |
| 热卖排行 | Top 10 热卖商品柱状图（含价格、销量、库存悬停详情） |
...
|------|------|---------|---------|
| 普通用户 | 原价 | — | 灰白 |
| 银卡会员 | 95折 | 累计消费 ¥1,000 | 金属银色 |
| 金卡会员 | 9折 | 累计消费 ¥5,000 | 亮金色 |
| 钻石会员 | 85折 | 累计消费 ¥20,000 | 宇宙紫 + 光晕呼吸动画 + 流光扫过特效 |

- **下单时自动计算**：`payAmount = totalAmount × 折扣率`，订单详情展示原价、折扣金额和实付金额
- **支付后自动升级**：支付成功时汇总用户全部已付订单累计消费金额，达到门槛自动晋升并写入操作日志
- **商品详情页会员价**：已登录用户实时查看自己等级对应的专属优惠价

### 5. 推荐算法优化

- **随机补充机制**：协同过滤推荐不足 8 个时，自动从在售商品中随机抽取补充，排除已购和已推荐商品，保证每次展示满 8 件
- **双端覆盖**：Web 端和小程序端均通过 `GET /api/product/recommend` 接口获得个性化推荐

### 6. 管理后台数据统计

- **营收统计卡片**：今日 GMV、今日实收、本月实收、本月退款，实时从订单表汇总计算
- **7 日趋势折线图**：商品/店铺/订单/用户四条曲线，每 30 秒自动刷新（ECharts）
- **订单状态饼图**：7 种状态分组（待支付/待发货/已发货/待评价/已完成/已取消/退款中）环形占比
- **热卖 Top 10 柱状图**：横向柱状图，橙色渐变配色，悬停显示商品详情（价格/销量/库存）
...
<p align="center">
  <sub>Built with ❤️ by PetShop Team · 2026</sub>
</p>
